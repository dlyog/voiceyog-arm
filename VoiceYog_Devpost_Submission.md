...
Predicted MOS is about naturalness, though, not about whether the words survive. So I also scored intelligibility: transcribing the model's output with ASR and comparing it against the input text gives **WER 0.0347** and **CER 0.0316**, with 8 of 10 sentences transcribed exactly, on that same checkpoint. Under three and a half percent word error says the compromise is one of polish rather than content — the words come through intact, and what the smaller model gives up is in the delivery.

Three honest caveats. UTMOS is an automatic predictor, not a human listening panel — it scores about 3.3 on pure silence, so the absolute number flatters and only the delta is trustworthy. The WER figure came from a separate internal evaluation harness rather than from this repository, so unlike the benchmark numbers it is not something you can re-run from what I have published; treat it as author-reported. And my family and I have listened to plenty of it and find it perfectly good for narration, tutorials or a personal voice — but that is an impression from an unblinded group of four, not a study, and it should count for nothing. What is still genuinely missing is a speaker-similarity score and a panel of 20–50 independent listeners. That is future work.
